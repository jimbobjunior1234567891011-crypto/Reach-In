# Builds reachin-<version>.jar without Gradle.
#
# NeoForge 21.1.x runs on Mojang mappings, and a CurseForge/NeoForge install already
# has every jar needed to compile against: the patched client, the universal jar, the
# Mojmap Minecraft jar and the full library set (Mixin included). So javac + jar is
# enough, and no refmap is needed.
...
# -proc:none keeps Mixin's annotation processor from demanding obfuscation mappings
# to build a refmap. NeoForge 21.1 runs on Mojang mappings, so names resolve directly
# and no refmap is wanted.
& "$Jdk\bin\javac.exe" -nowarn -proc:none -d $build -cp ($classpath -join ";") $sources
if ($LASTEXITCODE -ne 0) { throw "compile failed" }

Copy-Item -Recurse -Force (Join-Path $root "res\*") $build
& "$Jdk\bin\jar.exe" --create --file (Join-Path $dist "reachin-$Version.jar") -C $build .
if ($LASTEXITCODE -ne 0) { throw "jar failed" }

Write-Output "built $dist\reachin-$Version.jar"
